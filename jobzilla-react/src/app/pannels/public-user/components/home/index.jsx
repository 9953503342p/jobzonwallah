...
function Home1Page() {
  ...
  useEffect(() => {
    const fetchLatestJobs = async () => {
      try {
        const response = await fetch('http://localhost:8080/latest-jobs'); // Replace with your API URL
        if (!response.ok) {
        ...
        {jobs.map((job, index) => (
          <li key={index}>
            <div className="twm-jobs-list-style1 mb-5">
              <div className="twm-media">
                <img src={`http://localhost:8080/${job.userId.companylogo} `|| "default-image.jpg"} alt="Company" />
              </div>
